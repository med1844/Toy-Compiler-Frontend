...
def _defp0(defp, type_, id_):
    defp.node = ASTNode(
        "defParam", type_.node, ASTNode(id_.getContent(), actionID="id_func_var")
    )
...
def _forblock0(
    forBlock, for_, lp, forinit, semi0, forexpr0, semi1, forexpr1, rp, lcb, stmtl, rcb
):
    forBlock.node = ASTNode(
        "forBlock", forinit.node, forexpr0.node, stmtl.node, forexpr1.node
    )
...
def _func0(func, id_, lp, pal, rp):
    func.node = ASTNode(
        "functionCall", ASTNode(id_.getContent(), actionID="noAction"), pal.node
    )
...
class CodeBlock:
    ...
    def getSegments(self):
        self.calcSegment()
        newCBs = [self] + [
            CodeBlock(self.name + "_SEG_%d" % i) for i in range(1, self.segment[-1] + 1)
        ]
...
nsInheritance = (
    {}
)  # record the inheritance relationship between namespaces, for ID look up
    ...
    if "dec" in id_:
        ...
        while True:
            ...
            if ns not in nsInheritance:
                print(
                    "[ERROR] Undefined variable: %s has not been declared yet."
                    % varName
                )
                exit()
            ns = nsInheritance[ns]
...
class Function:
    ...
    def __str__(self):
        return "%s(%s)" % (
            self.name,
            ", ".join("%s %s" % (var.type, var.name) for var in self.parameters),
        )
...
def _funcDec(func, type_, func_id, defpl, sl):
    ...
    declaredFunctions[funcName] = Function(
        str(type_.getContent()), funcName, defpl.list, sl.cb
    )
