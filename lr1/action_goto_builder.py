...
class ActionGotoBuilder:
    @staticmethod
    def new(cfg: ContextFreeGrammar, lr_item_set_automata: LRItemSetAutomata) -> Tuple[Action, Goto]:
        action, goto = Action(cfg, len(lr_item_set_automata.item_set_to_id)), Goto(cfg, len(lr_item_set_automata.item_set_to_id))
